...
/// the map keeps exactly the pixel density it was designed at.
const PX: f32 = 2.0;
...
const fn fade(col: Rgba, alpha: f32) -> Rgba {
    Rgba {
        a: col.a * alpha,
        ..col
    }
...
const fn dim(col: Rgba, by: f32) -> Rgba {
    Rgba {
        r: col.r * by,
        g: col.g * by,
        b: col.b * by,
...
fn phosphorize(col: Rgba, amount: f32) -> Rgba {
...
fn omen_tint(col: Rgba, light: f32, omen: f32) -> Rgba {
    let cast = omen * 0.35;
    Rgba {
...
impl Canvas {
    ...
    const fn mood(&mut self, light: f32, omen: f32) {
        self.light = light;
        self.omen = omen;
    }
    ...
    fn blend(&mut self, column: u32, row: u32, col: Rgba, alpha: f32) {
    ...
    fn dot(&mut self, at: SimVec2, r: f32, col: Rgba) {
    ...
    fn ring(&mut self, at: SimVec2, r: f32, w: f32, col: Rgba) {
    ...
    fn seg(&mut self, a: SimVec2, b: SimVec2, w: f32, col: Rgba) {
        let half = Self::stroke(w) * 0.5;
        let h2 = half * half;
        self.paint(
            a.x.min(b.x) - half,
    ...
    fn tri(&mut self, a: SimVec2, b: SimVec2, d: SimVec2, col: Rgba) {
        self.paint(
            a.x.min(b.x).min(d.x),
            a.y.min(b.y).min(d.y),
            a.x.max(b.x).max(d.x),
    ...
    #[allow(dead_code)]
    fn tri_ring(&mut self, a: SimVec2, b: SimVec2, d: SimVec2, w: f32, col: Rgba) {
        self.seg(a, b, w, col);
        self.seg(b, d, w, col);
        self.seg(d, a, w, col);
    }
    ...
    fn poly(&mut self, at: SimVec2, sides: u8, r: f32, rot: f32, col: Rgba) {
    ...
    fn poly_ring(&mut self, at: SimVec2, sides: u8, r: f32, rot: f32, w: f32, col: Rgba) {
        let corners = ngon(at, sides, r, rot);
        let n = usize::from(sides).min(MAX_SIDES);
        for i in 0..n {
    ...
    fn arc(&mut self, at: SimVec2, r: f32, rot: f32, sweep: f32, w: f32, col: Rgba) {
    ...
    fn oval(&mut self, at: SimVec2, rx: f32, ry: f32, rot: f32, col: Rgba) {
    ...
    fn oval_ring(&mut self, at: SimVec2, rx: f32, ry: f32, rot: f32, w: f32, col: Rgba) {
        let half = Self::stroke(w) * 0.5;
...
fn ngon(at: SimVec2, sides: u8, r: f32, rot: f32) -> [SimVec2; MAX_SIDES] {
...
fn side(a: SimVec2, b: SimVec2, p: SimVec2) -> f32 {
    (b.x - a.x).mul_add(p.y - a.y, -((b.y - a.y) * (p.x - a.x)))
}

/// Squared distance from `p` to segment `a b`.
fn seg_dist2(a: SimVec2, b: SimVec2, p: SimVec2) -> f32 {
    let ab = b - a;
    let ap = p - a;
...
fn ellipse_level(at: SimVec2, rx: f32, ry: f32, sin: f32, cos: f32, sx: f32, sy: f32) -> f32 {
    let dx = sx - at.x;
    let dy = sy - at.y;
...
fn polar(from: SimVec2, radius: f32, angle: f32) -> SimVec2 {
    SimVec2::new(
...
const fn rect_center(r: Rect) -> SimVec2 {
