//! Embed a `git describe`-based version at compile time — same contract as
//! the workspace root's build.rs, with rerun paths aimed at the repo root's
//! `.git` from this crate's directory.
